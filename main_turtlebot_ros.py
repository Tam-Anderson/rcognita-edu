...
class ROS_preset:
    ...
    def odometry_callback(self, msg):
        ...
        state_matrix = np.vstack([self.state_goal[0], self.state_goal[1], self.state_goal[2]])  # [x, y, 0] -- column   
    ...
    def spin(self, is_print_sim_step=False, is_log_data=False):
        ...
        while not rospy.is_shutdown() and datetime.now() - start_time < timedelta(100):
            ...
            if is_print_sim_step:
                self.logger.print_sim_step(t, self.new_state[0], self.new_state[1], self.new_state[2], run_obj, accum_obj, action) #self.logger.print_sim_step('''t, xCoord, yCoord ...''')
            
            if is_log_data:
                self.logger.log_data_row(self.datafiles[0], t, self.new_state[0], self.new_state[1], self.new_state[2], run_obj, accum_obj, action) # self.logger.log_data_row('''t, xCoord, yCoord ...''')
